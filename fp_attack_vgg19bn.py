...
parser.add_argument('-b', '--batch_size', default=20, type=int, metavar='N', help='mini-batch size (default: 50)')
...
@torch.no_grad()
def test(x_adv, y, target_y, Vgg_19, Inc_v3, DenseNet_121, ResNet_50, list_vgg, list_y, num, utr, tsuc, ttr):
    x_adv = x_adv.cuda()
    y = y.cuda()
    ...
    # White Box Model
    num[0] += torch.sum(pred_adv_vgg  != y)
    tsuc[0] += torch.sum(pred_adv_vgg  == target_y)
    idx_vgg = pred_adv_vgg != y
    idx_vgg_t = pred_adv_vgg == target_y

    # Save White Box Model Tsuc List
    for img in x_adv[idx_vgg_t]:
        list_vgg.append(img.detach().cpu().numpy())
    for t_y in target_y[idx_vgg_t]:
        list_y.append(t_y.detach().cpu().numpy())
    
    # Black Box Model
    num[1] += torch.sum(pred_adv_incv3 != y)
    tsuc[1] += torch.sum(pred_adv_incv3 == target_y)
    ...
    utr[0] += torch.sum(idx_vgg & idx_incv3)
    utr[1] += torch.sum(idx_vgg & idx_121)
    utr[2] += torch.sum(idx_vgg & idx_50)
    ttr[0] += torch.sum(idx_vgg_t & idx_incv3_t)
    ttr[1] += torch.sum(idx_vgg_t & idx_121_t)
    ttr[2] += torch.sum(idx_vgg_t & idx_50_t)

    return list_vgg, list_y, num, utr, tsuc, ttr 
...
def main():
    ...
    for (name, module) in vgg_19.named_modules():
        if isinstance(module, nn.MaxPool2d):
            vgg19_layer_list.append(name)
        if isinstance(module, nn.Conv2d):
            vgg19_layer_list.append(name)
        if isinstance(module, nn.Linear):
            vgg19_layer_list.append(name) 
    
     # from ori paper's 2st layer [256]
    vgg19_layer_list = ['1.features.14', '1.features.17', '1.features.20', '1.features.23', '1.features.26', '1.features.27', '1.features.30', '1.features.33', '1.features.36', '1.features.39', '1.features.40', '1.features.43', '1.features.46', '1.features.49', '1.features.52', '1.classifier.0', '1.classifier.3', '1.classifier.6']

    # for gussi kernel
    vgg19_layer_list = ['1.features.26', '1.features.27', '1.features.30', '1.features.33', '1.features.36', '1.features.39', '1.features.40', '1.features.43', '1.features.46', '1.features.49', '1.features.52', '1.classifier.0', '1.classifier.3', '1.classifier.6']
    batch_size = [2,1,1,1,1,10,10,10,10,10,70,8,8,70]

    # for gussi kernel, random class
    vgg19_layer_list = ['1.features.40']
    batch_size = [10]
    ...
    print('c: ', c)
    print('args.targetcls: ', args.targetcls)
    ba_s=0
    
    for layer_name in vgg19_layer_list:
        handlers = []
        for (name, module) in vgg_19.named_modules():
            if name == layer_name:
                print('\n')
        ...
        for i, (x, y) in (enumerate(val_loader)):
            ...
            if i != args.num // batch_size[ba_s]:
                ...
                with torch.no_grad():
                    ...
                    for j in target_y:
                        target_out = vgg_19(library[j].cuda())  
                ...
                x_adv = attack_fp(x, furthest_feature, vgg_19, kernel_type, bn_loss, c, batch_size[ba_s]).detach_()
                ...
                list_vgg, list_target_y, num, utr, tsuc, ttr = test(x_adv, y, target_y, vgg_19, inc_v3, densenet_121, resnet_50, list_vgg, list_y, num, utr, tsuc, ttr)
            else:
                break

        D_adv = float(args.num)
        
        print('Error for vgg19: %10.4f' % float(100 * (float(num[0]) / D_adv)))
        print('tSuc for vgg19: %10.4f' % float(100 * (float(tsuc[0]) / D_adv)))

        print('Error for inc_v3: %10.4f' % float(100 * (float(num[1]) / D_adv)))
        print('uTR for inc_v3: %10.4f' % float(100 * (float(utr[0]) / float(num[0]))))
        print('tSuc for inc_v3: %10.4f' % float(100 * (float(tsuc[1]) / D_adv)))
        if tsuc[0]<1e-20:
            print('tTR for inc_v3: %10.4f' % float(100 * (0.0)))
        else:
            print('tTR for inc_v3: %10.4f' % float(100 * (float(ttr[0]) / float(tsuc[0]))))
        ...
        if tsuc[0]<1e-20:
            print('tTR for dense121: %10.4f' % float(100 * (0.0)))
        else:
