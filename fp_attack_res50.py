...
def main():
    ...
    for (name, module) in resnet_50.named_modules():
        if isinstance(module, models.resnet.Bottleneck):
            res50_layer_list.append(name)
        if isinstance(module, nn.Linear):
            res50_layer_list.append(name)

    # res50_layer_list = ['1.layer1.0', '1.layer1.1', '1.layer1.2', '1.layer2.0', '1.layer2.1', '1.layer2.2', '1.layer2.3', '1.layer3.0', '1.layer3.1', '1.layer3.2', '1.layer3.3', '1.layer3.4', '1.layer3.5', '1.layer4.0', '1.layer4.1', '1.layer4.2', '1.fc']

    # for gussi kernel
    res50_layer_list = ['1.layer2.0', '1.layer2.1', '1.layer2.2', '1.layer2.3', '1.layer3.0', '1.layer3.1', '1.layer3.2', '1.layer3.3', '1.layer3.4', '1.layer3.5', '1.layer4.0', '1.layer4.1', '1.layer4.2', '1.fc']
    batch_size = [1,1,1,1,6,6,6,6,6,6,40,40,40,80]

    # for gussi kernel, random class
    res50_layer_list = ['1.layer3.4']
    batch_size = [6]
    ...
    for layer_name in res50_layer_list:
        handlers = []
        for (name, module) in resnet_50.named_modules():
            if name == layer_name:
                print('\n')
        ...
        for i, (x, y) in (enumerate(val_loader)):
            ...
            if i != args.num // batch_size[ba_s]:
                ...
                with torch.no_grad():
                    ...
                    for j in target_y:
                        target_out = resnet_50(library[j].cuda())  
                ...
                x_adv = attack_fp(x, furthest_feature, resnet_50, kernel_type, bn_loss, c, batch_size[ba_s]).detach_()
