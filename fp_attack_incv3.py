...
parser.add_argument('--d', type=int, default=2,  help='the param of poly kernel')
...
def MMD_poly(source, target, c):
    ...
    diffx = torch.sum((source.transpose(1,2).bmm(source))**d, axis=(1,2)) + 2*c*torch.sum((source.transpose(1,2).bmm(source)), axis=(1,2))
    diffy = torch.sum((target.transpose(1,2).bmm(target))**d, axis=(1,2)) + 2*c*torch.sum(target.transpose(1,2).bmm(target), axis=(1,2)) 
    diffxy = torch.sum((source.bmm(target.transpose(1,2)))**d, axis=(1,2)) + 2*c*torch.sum(source.bmm(target.transpose(1,2)), axis=(1,2))
...
def MMD_linearTime_poly(source, target, c):
    ...
    diffx = torch.sum((x_even.transpose(1,2).bmm(x_odd))**d, axis=(1,2)) + 2*c*torch.sum((x_even.transpose(1,2).bmm(x_odd)), axis=(1,2))
    diffy = torch.sum((y_even.transpose(1,2).bmm(y_odd))**d, axis=(1,2)) + 2*c*torch.sum(y_even.transpose(1,2).bmm(y_odd), axis=(1,2)) 
    diffxy = torch.sum((x_even.bmm(y_odd.transpose(1,2)))**d, axis=(1,2)) + 2*c*torch.sum(x_even.bmm(y_odd.transpose(1,2)), axis=(1,2))
    diffyx = torch.sum((x_odd.bmm(y_even.transpose(1,2)))**d, axis=(1,2)) + 2*c*torch.sum(x_odd.bmm(y_even.transpose(1,2)), axis=(1,2))
...
@torch.no_grad()
def test(x_adv, y, target_y, Inc_v3, Vgg_19, DenseNet_121, ResNet_50, list_incv3, list_y, num, utr, tsuc, ttr):
    x_adv = x_adv.cuda()
    y = y.cuda()
    ...
    # White Box Model
    num[0] += torch.sum(pred_adv_incv3 != y)
    tsuc[0] += torch.sum(pred_adv_incv3 == target_y)
    idx_incv3 = pred_adv_incv3 != y
    idx_incv3_t = pred_adv_incv3 == target_y

    # Save White Box Model Tsuc List
    for img in x_adv[idx_incv3_t]:
        list_incv3.append(img.detach().cpu().numpy())
    for t_y in target_y[idx_incv3_t]:
    ...
    num[2] += torch.sum(pred_adv_121 != y)
    tsuc[2] += torch.sum(pred_adv_121 == target_y)
    ...
    utr[0] += torch.sum(idx_incv3 & idx_vgg)
    utr[1] += torch.sum(idx_incv3 & idx_121)
    utr[2] += torch.sum(idx_incv3 & idx_50)
    ttr[0] += torch.sum(idx_incv3_t & idx_vgg_t)
    ttr[1] += torch.sum(idx_incv3_t & idx_121_t)
    ttr[2] += torch.sum(idx_incv3_t & idx_50_t)

    return list_incv3, list_y, num, utr, tsuc, ttr 
...
def main():
    ...
    root = "/home/chengyaya/data/code_attack/Non-Targeted-Adversarial-Attacks/test_stylemmd1"
    ...
    # incv3_layer_list = ['1.Conv2d_1a_3x3', '1.Conv2d_2a_3x3', '1.Conv2d_2b_3x3', '1.maxpool1', '1.Conv2d_3b_1x1', '1.Conv2d_4a_3x3', '1.maxpool2', '1.Mixed_5b', '1.Mixed_5c', '1.Mixed_5d', '1.Mixed_6a', '1.Mixed_6b', '1.Mixed_6c', '1.Mixed_6d', '1.Mixed_6e', '1.AuxLogits', '1.Mixed_7a', '1.Mixed_7b', '1.Mixed_7c', '1.avgpool', '1.fc']
    # for faster
    ...
    # for gussi kernel
    incv3_layer_list = ['1.maxpool2', '1.Mixed_5b', '1.Mixed_5c', '1.Mixed_5d', '1.Mixed_6a', '1.Mixed_6b', '1.Mixed_6c', '1.Mixed_6d', '1.Mixed_6e', '1.Mixed_7a', '1.Mixed_7b', '1.Mixed_7c', '1.avgpool', '1.fc']
    batch_size=[4,3,2,2,15,15,15,15,15,80,90,90,150,100]

    # for gussi kernel, random class
    incv3_layer_list = ['1.Mixed_6c', '1.Mixed_6d', '1.Mixed_6e', '1.Mixed_7a', '1.Mixed_7b', '1.Mixed_7c', '1.avgpool', '1.fc']
    batch_size=[15]
        ...
        for i, (x, y) in (enumerate(val_loader)):
            ...
            if i != args.num // batch_size[ba_s]:
                ...
                with torch.no_grad():
                    ...
                    for j in target_y:
                        target_out = inc_v3(library[j].cuda())  
                    ...
                    # furthest_feature = torch.ones_like(source_feature)
                
                x_adv = attack_fp(x, furthest_feature, inc_v3, kernel_type, bn_loss, c, batch_size[ba_s]).detach_()
                
                # save adv img
                # save_path = os.path.join(root, str(i))
                # save_img(save_path, x_adv)
                
                # x_adv = attack_di(x, torch.from_numpy(target_y), densenet_121).detach_()
